...
public class VozaciDispecerForma extends JFrame {
	...
	private void initGUI() {
		...
		for (Automobil automobil : taksiSluzba.sviNeobrisaniAutomobili()) {
			cbAutomobili.addItem(automobil.getIdAutomobila());
		}
		...
		btnOK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (validacija()) {
					...
					Automobil automobil = taksiSluzba.getAutomobilPoID(automobilID);
					
					//DODAVANJE
					if (vozac == null) {
						Vozac noviVozac = new Vozac(redniBrojKorisnika, korisnickoIme, lozinka, ime, prezime, JMBG, adresa, pol, brojTelefona, false, plata, brojClanskeKarte, automobil);
						taksiSluzba.dodajVozaca(noviVozac);
					} else {   //IZMENA
						
						vozac.setIdKorisnika(redniBrojKorisnika);
